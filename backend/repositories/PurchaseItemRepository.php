...
final class PurchaseItemRepository{
 ...
 public function returnable(int$purchase):array{$s=$this->db->connection()->prepare('SELECT pi.*,pi.quantity_base AS quantity,p.name current_product_name,p.stock_quantity_base current_stock,p.track_stock,EXISTS(SELECT 1 FROM stock_transactions st WHERE st.reference_type=\'purchase\' AND st.reference_id=pi.purchase_id AND st.product_id=pi.product_id) stock_posted FROM purchase_items pi JOIN products p ON p.id=pi.product_id WHERE pi.purchase_id=:id ORDER BY pi.id FOR UPDATE');$s->execute(['id'=>$purchase]);$r=[];foreach($s->fetchAll()as$row)$r[(int)$row['id']]=$row;return$r;}
 ...
 public function remainingCount(int$purchase):int{$s=$this->db->connection()->prepare('SELECT COUNT(*) FROM purchase_items WHERE purchase_id=:id AND returned_quantity<quantity_base');$s->execute(['id'=>$purchase]);return(int)$s->fetchColumn();}
}
