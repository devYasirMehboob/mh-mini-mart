...
final class ReportRepository
{
    ...
    public function overviewExtras(array $f): array
    {
        ...
        $category=$this->first("SELECT c.name,SUM(si.line_total-CASE WHEN s.subtotal>0 THEN si.line_total/s.subtotal*s.discount_amount ELSE 0 END) net_sales FROM sale_items si JOIN sales s ON s.id=si.sale_id JOIN products p ON p.id=si.product_id JOIN categories c ON c.id=p.category_id".$categoryWhere." AND s.status='completed' GROUP BY c.id,c.name ORDER BY net_sales DESC LIMIT 1",$categoryParams);
    ...
    public function products(array $f,bool $bestOnly=false): array
    {
        ...
        $select="SELECT si.product_id,si.product_name,si.product_code,c.name category_name,p.unit_type,SUM(si.quantity) quantity_sold,COUNT(*) sale_lines,SUM(si.line_total) gross_sales,SUM(CASE WHEN s.subtotal>0 THEN si.line_total/s.subtotal*s.discount_amount ELSE 0 END) discounts,SUM(si.line_total-CASE WHEN s.subtotal>0 THEN si.line_total/s.subtotal*s.discount_amount ELSE 0 END) net_sales,SUM(si.purchase_cost*si.quantity) cost_of_goods,SUM(si.line_total-CASE WHEN s.subtotal>0 THEN si.line_total/s.subtotal*s.discount_amount ELSE 0 END-si.purchase_cost*si.quantity) gross_profit,AVG(si.unit_price) average_selling_price,p.quantity current_stock".$base.$group;
        $rows=$this->paged($select." ORDER BY $sort $direction,si.product_name ASC LIMIT :limit OFFSET :offset",$params,$f);
        $sum=$this->first("SELECT COALESCE(SUM(si.quantity),0) quantity_sold,COALESCE(SUM(si.line_total),0) gross_sales,COALESCE(SUM(si.line_total-CASE WHEN s.subtotal>0 THEN si.line_total/s.subtotal*s.discount_amount ELSE 0 END),0) net_sales,COALESCE(SUM(si.purchase_cost*si.quantity),0) cost_of_goods".$base,$params)?:[];$sum['gross_profit']=$this->decimal((float)($sum['net_sales']??0)-(float)($sum['cost_of_goods']??0));
    ...
    public function categories(array $f): array
    {
        ...
        $summary=$this->first("SELECT COALESCE(SUM(si.line_total-CASE WHEN s.subtotal>0 THEN si.line_total/s.subtotal*s.discount_amount ELSE 0 END),0) net_sales".$base,$params)?:[];$sort=['quantity'=>'quantity_sold','net_sales'=>'net_sales','gross_sales'=>'gross_sales','gross_profit'=>'gross_profit','name'=>'category_name'][$f['sort_by']]??'net_sales';$direction=$f['sort_direction']==='asc'?'ASC':'DESC';
        $select="SELECT c.id category_id,c.name category_name,SUM(si.quantity) quantity_sold,COUNT(DISTINCT si.product_id) products_sold,COUNT(DISTINCT s.id) sales_count,SUM(si.line_total) gross_sales,SUM(CASE WHEN s.subtotal>0 THEN si.line_total/s.subtotal*s.discount_amount ELSE 0 END) discounts,SUM(si.line_total-CASE WHEN s.subtotal>0 THEN si.line_total/s.subtotal*s.discount_amount ELSE 0 END) net_sales,SUM(si.purchase_cost*si.quantity) cost_of_goods,SUM(si.line_total-CASE WHEN s.subtotal>0 THEN si.line_total/s.subtotal*s.discount_amount ELSE 0 END-si.purchase_cost*si.quantity) gross_profit".$base.$group;
