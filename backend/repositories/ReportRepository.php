...
final class ReportRepository
{
    ...
    public function financialSummary(array $f): array
    {
        ...
        $sql="SELECT
            ...
            COALESCE(SUM(CASE WHEN s.status='completed' THEN (SELECT COALESCE(SUM(si.purchase_cost*si.quantity_base),0) FROM sale_items si WHERE si.sale_id=s.id) ELSE 0 END),0) cost_of_goods,
    ...
    public function overviewExtras(array $f): array
    {
        ...
        $units=$this->scalar("SELECT COALESCE(SUM(si.quantity_base),0) FROM sale_items si JOIN sales s ON s.id=si.sale_id".$where." AND s.status='completed'",$params);
        $best=$this->first("SELECT si.product_name name,si.product_code code,SUM(si.quantity_base) quantity_sold FROM sale_items si JOIN sales s ON s.id=si.sale_id".$where." AND s.status='completed' GROUP BY si.product_id,si.product_name,si.product_code ORDER BY quantity_sold DESC LIMIT 1",$params);
        ...
        $stock=$this->first("SELECT COALESCE(SUM(track_stock=1 AND stock_quantity_base>0 AND stock_quantity_base<=minimum_stock),0) low_stock_count,COALESCE(SUM(track_stock=1 AND stock_quantity_base<=0),0) out_of_stock_count FROM products",[]);
    ...
    public function products(array $f,bool $bestOnly=false): array
    {
        ...
        $base=" FROM sale_items si JOIN sales s ON s.id=si.sale_id JOIN products p ON p.id=si.product_id JOIN categories c ON c.id=p.category_id LEFT JOIN units u ON u.id=p.base_unit_id".$where;
        $group=' GROUP BY si.product_id,si.product_name,si.product_code,c.name,COALESCE(u.name, \'piece\'),p.stock_quantity_base';
        ...
        $select="SELECT si.product_id,si.product_name,si.product_code,c.name category_name,COALESCE(u.name, 'piece') AS unit_type,SUM(si.quantity_base) quantity_sold,COUNT(*) sale_lines,SUM(si.line_total) gross_sales,SUM(si.discount_amount) discounts,SUM(si.line_total-si.discount_amount) net_sales,SUM(si.purchase_cost*si.quantity_base) cost_of_goods,SUM(si.line_total-si.discount_amount-si.purchase_cost*si.quantity_base) gross_profit,AVG(si.unit_price) average_selling_price,p.stock_quantity_base current_stock".$base.$group;
        $rows=$this->paged($select." ORDER BY $sort $direction,si.product_name ASC LIMIT :limit OFFSET :offset",$params,$f);
        $sum=$this->first("SELECT COALESCE(SUM(si.quantity_base),0) quantity_sold,COALESCE(SUM(si.line_total),0) gross_sales,COALESCE(SUM(si.line_total-si.discount_amount),0) net_sales,COALESCE(SUM(si.purchase_cost*si.quantity_base),0) cost_of_goods".$base,$params)?:[];$sum['gross_profit']=$this->decimal((float)($sum['net_sales']??0)-(float)($sum['cost_of_goods']??0));
    ...
    public function categories(array $f): array
    {
        ...
        $select="SELECT c.id category_id,c.name category_name,SUM(si.quantity_base) quantity_sold,COUNT(DISTINCT si.product_id) products_sold,COUNT(DISTINCT s.id) sales_count,SUM(si.line_total) gross_sales,SUM(si.discount_amount) discounts,SUM(si.line_total-si.discount_amount) net_sales,SUM(si.purchase_cost*si.quantity_base) cost_of_goods,SUM(si.line_total-si.discount_amount-si.purchase_cost*si.quantity_base) gross_profit".$base.$group;
    ...
    {
        [$where,$params]=$this->stockWhere($f,'st',$mode);$total=(int)$this->scalar('SELECT COUNT(*) FROM products p JOIN categories c ON c.id=p.category_id'.$where,$params);$sort=['name'=>'p.name','stock'=>'p.stock_quantity_base','quantity'=>'p.stock_quantity_base','shortage'=>'shortage','category'=>'c.name','status'=>'stock_status'][$f['sort_by']]??'p.name';$direction=$f['sort_direction']==='asc'?'ASC':'DESC';
        $select="SELECT p.id product_id,p.name product_name,p.product_code,c.name category_name,p.stock_quantity_base current_quantity,p.minimum_stock,COALESCE(u.name, 'piece') AS unit_type,p.track_stock,p.status product_status,p.purchase_cost,p.stock_quantity_base*p.purchase_cost estimated_stock_value,CASE WHEN p.track_stock=0 THEN 'not_tracked' WHEN p.stock_quantity_base<=0 THEN 'out_of_stock' WHEN p.stock_quantity_base<=p.minimum_stock THEN 'low_stock' ELSE 'in_stock' END stock_status,GREATEST(p.minimum_stock-p.stock_quantity_base,0) shortage,(SELECT MAX(st.created_at) FROM stock_transactions st WHERE st.product_id=p.id) last_movement";
        $rows=$this->paged($select.' FROM products p JOIN categories c ON c.id=p.category_id LEFT JOIN units u ON u.id=p.base_unit_id'.$where." ORDER BY $sort $direction,p.id LIMIT :limit OFFSET :offset",$params,$f);$summary=$this->first("SELECT COUNT(*) product_count,COALESCE(SUM(p.track_stock=1 AND p.stock_quantity_base>0 AND p.stock_quantity_base<=p.minimum_stock),0) low_stock_count,COALESCE(SUM(p.track_stock=1 AND p.stock_quantity_base<=0),0) out_of_stock_count,COALESCE(SUM(CASE WHEN p.track_stock=1 THEN p.stock_quantity_base*p.purchase_cost ELSE 0 END),0) stock_value FROM products p JOIN categories c ON c.id=p.category_id".$where,$params)?:[];
        $chart=$this->all("SELECT CASE WHEN p.track_stock=0 THEN 'not_tracked' WHEN p.stock_quantity_base<=0 THEN 'out_of_stock' WHEN p.stock_quantity_base<=p.minimum_stock THEN 'low_stock' ELSE 'in_stock' END label,COUNT(*) total FROM products p JOIN categories c ON c.id=p.category_id".$where.' GROUP BY label ORDER BY total DESC',$params);return$this->pageResult($rows,$total,$f,$summary,$chart);
    }
    ...
    {
        $parts=[];$params=[];if($f['search']!==''){$parts[]="(p.name LIKE :{$prefix}_name OR p.product_code LIKE :{$prefix}_code OR p.barcode LIKE :{$prefix}_barcode OR c.name LIKE :{$prefix}_category_name)";$pattern='%'.$f['search'].'%';foreach(['name','code','barcode','category_name']as$key)$params[$prefix.'_'.$key]=$pattern;}if($f['category_id']!==null){$parts[]="p.category_id=:{$prefix}_category";$params[$prefix.'_category']=$f['category_id'];}if($f['tracking']==='tracked')$parts[]='p.track_stock=1';elseif($f['tracking']==='untracked')$parts[]='p.track_stock=0';if($mode==='low')$parts[]='p.track_stock=1 AND p.stock_quantity_base>0 AND p.stock_quantity_base<=p.minimum_stock';elseif($mode==='out')$parts[]='p.track_stock=1 AND p.stock_quantity_base<=0';elseif($f['stock_status']==='low_stock')$parts[]='p.track_stock=1 AND p.stock_quantity_base>0 AND p.stock_quantity_base<=p.minimum_stock';elseif($f['stock_status']==='out_of_stock')$parts[]='p.track_stock=1 AND p.stock_quantity_base<=0';elseif($f['stock_status']==='in_stock')$parts[]='p.stock_quantity_base>p.minimum_stock';return[$parts?' WHERE '.implode(' AND ',$parts):'',$params];
    }
