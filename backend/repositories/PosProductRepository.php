...
final class PosProductRepository
{
    ...
    public function paginate(array $filters): array
    {
        ...
        if ($filters['search'] !== '') {
            ...
            $where[] = '(p.name LIKE :name OR p.product_code LIKE :code OR p.barcode LIKE :barcode)';
            $parameters = ['name' => $pattern, 'code' => $pattern, 'barcode' => $pattern];
        }
        ...
        if ($filters['ids'] !== []) {
            ...
            $where[] = 'p.id IN (' . implode(',', $holders) . ')';
        }
        $whereSql = ' WHERE ' . implode(' AND ', $where);
        $count = $this->database->connection()->prepare('SELECT COUNT(*) FROM products p' . $whereSql);
        $count->execute($parameters);
        $total = (int)$count->fetchColumn();
        
        $statement = $this->database->connection()->prepare(
            'SELECT p.id, p.category_id, c.name AS category_name, p.name, p.product_code, p.barcode, p.selling_price, 
             IF(p.stock_mode = \'shared\', IFNULL(FLOOR(source.stock_quantity_base / p.consumption_quantity_base), 0), p.stock_quantity_base) AS quantity,
        ...
        return [
            ...
            'pagination' => ['page' => $filters['page'], 'limit' => $filters['limit'], 'total' => $total, 'total_pages' => $total === 0 ? 0 : (int)ceil($total / $filters['limit'])]
        ];
    }
    ...
    public function findMany(array $ids): array
    {
        ...
        foreach (array_values($ids) as $i => $id) $s->bindValue($i + 1, $id, PDO::PARAM_INT);
        $s->execute();
        $result = [];
        foreach ($s->fetchAll() as $product) $result[(int)$product['id']] = $product;
        return $result;
    }
    
    public function activeCategories(): array
    {
        $s = $this->database->connection()->prepare("SELECT id,name FROM categories WHERE status='active' ORDER BY name");
        $s->execute();
        return $s->fetchAll();
    }
}
