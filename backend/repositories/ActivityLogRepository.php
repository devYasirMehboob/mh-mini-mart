...
final class ActivityLogRepository
{
 ...
 public function getLatestAction(string $action): ?array { $s = $this->database->connection()->prepare('SELECT * FROM activity_logs WHERE action = :action ORDER BY created_at DESC LIMIT 1'); $s->execute(['action' => $action]); $row = $s->fetch(); return $row === false ? null : $row; }
}