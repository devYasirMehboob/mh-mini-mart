...
final class SaleService
{
    ...
    public function complete(int $cashierId,array $input,bool $isAdmin=false): array
    {
        ...
        try{if($data['held_sale_id']!==null){$heldSale=$this->held->findForUpdate($data['held_sale_id']);if($heldSale===null||$heldSale['status']!=='active')throw new HttpException('Held sale is no longer available.',409);if((int)$heldSale['held_by']!==$cashierId&&!$isAdmin)throw new HttpException('You do not have permission to complete this held sale.',403);}$locked=$this->products->findManyForUpdate(array_keys($data['items']));if(count($locked)!==count($data['items']))throw new HttpException('One or more products are no longer available.',409);$lines=[];$subtotal=0;
            ...
            $this->payments->create(['sale_id'=>$saleId,'payment_method'=>$data['payment_method'],'amount'=>$this->money($grand),'status'=>'paid','reference'=>$data['payment_reference']]);if($data['held_sale_id']!==null)$this->held->complete($data['held_sale_id'],$saleId);$pdo->commit();return['sale'=>$this->sales->findReceipt($saleId),'already_completed'=>false];
    ...
    public function cancel(array $user,int $saleId,array $input): array
    {
        ...
        try{$sale=$this->sales->findForUpdate($saleId);$this->assertEligible($sale,'cancel');$items=$this->items->findBySale($saleId,false);$this->restoreStock($items,(int)$user['id'],'cancel',$saleId,$sale['invoice_number'],$data['reason']);$this->sales->cancel($saleId,(int)$user['id'],$data['reason']);$pdo->commit();return$this->detail($user,$saleId);}catch(Throwable$exception){if($pdo->inTransaction())$pdo->rollBack();throw$exception;}
    }
    ...
    public function refund(array $user,int $saleId,array $input): array
    {
        ...
        try{$sale=$this->sales->findForUpdate($saleId);$this->assertEligible($sale,'refund');$items=$this->items->findBySale($saleId,false);$refundId=$this->refunds->create(['sale_id'=>$saleId,'processed_by'=>(int)$user['id'],'refund_amount'=>$sale['grand_total'],'refund_method'=>$data['refund_method'],'reason'=>$data['reason']]);$this->restoreStock($items,(int)$user['id'],'refund',$refundId,$sale['invoice_number'],$data['reason']);$this->payments->markRefunded($saleId);$this->sales->refund($saleId);$pdo->commit();return$this->detail($user,$saleId);}catch(Throwable$exception){if($pdo->inTransaction())$pdo->rollBack();throw$exception;}
    }
