...
const groups = [
  {
    ...
    items: [
      ...
      },
      {
        ...
        path: "/customers",
        icon: "users",
        description: "Profiles and Khata",
        permission: "pos.access",
      },
    ],
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        path: "/suppliers",
        icon: "users",
        description: "Vendors and balances",
...
function AppLayout() {
  ...
  ) || { label: "MH Mini Mart", description: "Store management" };
  ...
  return (
    <div className="min-h-screen bg-app text-ink">
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-40 flex flex-col border-r border-slate-200/80 bg-white p-4 transition-all duration-300 lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} ${isCollapsed ? "w-[280px] lg:w-[88px]" : "w-[280px]"}`}
      >
        ...
        <nav
          className="no-scrollbar flex flex-1 flex-col gap-5 overflow-y-auto overflow-x-hidden pr-1"
          aria-label="Main navigation"
        >
          {visibleGroups.map((group) => (
            <section key={group.label}>
              <p
                className={`px-3 mb-2 text-[10px] font-extrabold uppercase tracking-[0.14em] text-slate-400 transition-all duration-300 whitespace-nowrap overflow-hidden ${isCollapsed ? "lg:max-h-0 lg:opacity-0 lg:mb-0" : "max-h-[20px] opacity-100"}`}
              >
                {group.label}
              </p>
        ...
        <div
          className={`mt-4 rounded-2xl border transition-all duration-300 overflow-hidden shrink-0 ${isCollapsed ? "lg:bg-transparent lg:border-transparent lg:p-0 lg:w-12 lg:mx-auto" : "border-slate-200 bg-slate-50 p-3 w-full"}`}
        >
          <div
            className={`flex items-center transition-all duration-300 ${isCollapsed ? "lg:justify-center" : ""}`}
          >
            <span
              className="grid size-10 shrink-0 place-items-center rounded-xl bg-white text-xs font-extrabold text-blue-700 shadow-sm ring-1 ring-slate-200"
            ...
            >
              ...
              <span className="mt-0.5 block truncate text-[10px] capitalize text-slate-400">
                {user.role} account
              </span>
            </div>
            <button
              className={`grid size-9 shrink-0 place-items-center rounded-xl text-slate-400 transition-colors hover:bg-white hover:text-red-600 hover:shadow-sm overflow-hidden duration-300 ${isCollapsed ? "lg:max-w-0 lg:opacity-0 lg:ml-0 lg:p-0" : "max-w-[36px] opacity-100 ml-2"}`}
              type="button"
              aria-label="Logout"
              title="Logout"
      ...
      <div
        className={`min-h-screen transition-all duration-300 ${isCollapsed ? "lg:ml-[88px]" : "lg:ml-[280px]"}`}
      >
        <OfflineBanner />
          ...
          <div className="min-w-0 flex-1 flex items-center gap-4">
            ...
            <div className="flex-1 max-w-md ml-auto lg:ml-8">
              <button
                ...
                className="flex w-full items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-400 transition hover:bg-slate-100"
              >
                <Icon name="search" className="size-4" />
                <span className="flex-1 text-left">Search anything...</span>
                <kbd className="hidden rounded bg-white px-1.5 py-0.5 text-[10px] font-bold text-slate-500 ring-1 ring-slate-200 sm:inline-block">
                  ⌘K
                </kbd>
              </button>
            </div>
              ...
              <div className="relative">
                ...
                {addDropdownOpen && (
                  <>
                    ...
                    />
                    <div className="absolute right-0 top-full mt-2 w-56 z-50 rounded-xl border border-slate-200 bg-white p-2 shadow-xl">
                      ...
                      <NavLink
                        to="/customers"
                        onClick={() => setAddDropdownOpen(false)}
                        className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-semibold text-slate-600 transition hover:bg-slate-50 hover:text-blue-600"
                      >
                        <Icon name="users" className="size-4 text-blue-600" />{" "}
                        New Customer / Khata
                      </NavLink>
                      {can("pos.access") && (
                        <NavLink
                          to="/pos"
                          onClick={() => setAddDropdownOpen(false)}
                          className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-semibold text-slate-600 transition hover:bg-slate-50 hover:text-emerald-600"
                        >
                          <Icon
                            name="pos"
                      ...
                      {can("expenses.view") && (
                        <NavLink
                          to="/expenses"
                          onClick={() => setAddDropdownOpen(false)}
                          className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-semibold text-slate-600 transition hover:bg-slate-50 hover:text-purple-600"
                        >
                          <Icon
                            name="expenses"
                      ...
                      {can("products.view") && (
                        <NavLink
                          to="/products"
                          onClick={() => setAddDropdownOpen(false)}
                          className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-semibold text-slate-600 transition hover:bg-slate-50 hover:text-slate-900"
                        >
                          <Icon
                            name="products"
