...
function ProductCard({ product, onAdd }) {
  ...
  return (
    <button
      ...
      className="premium-surface group overflow-hidden rounded-xl text-left transition hover:-translate-y-0.5 hover:border-blue-200 hover:shadow-[0_12px_28px_rgba(16,24,40,0.08)] focus:outline-none focus:ring-4 focus:ring-blue-100 disabled:cursor-not-allowed disabled:opacity-60 disabled:hover:translate-y-0"
    >
      ...
      <span className="block p-3.5">
        ...
        <span className="mt-3 flex items-end justify-between gap-2">
          <span><strong className="block text-[15px] font-extrabold text-slate-950">{formatCurrency(product.selling_price)}</strong><small className="mt-0.5 block text-[10px] font-medium text-slate-400">{tracked ? `${Number(product.quantity)} ${product.unit_type} available` : "Stock not tracked"}</small></span>
